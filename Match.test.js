...
test('Indicar un ganador: ganador es jugador, la fecha del partido es hoy', () => {
  const today = new Date();
...
test('Indicar un ganador: ganador es jugador, la fecha del partido es en el futuro', () => {
  const futureDate = new Date('2023/10/24');
...
test('Indicar un ganador: ganador es jugador, el jugador no pertenece al partido', () => {
...
test('Indicar un ganador: ganador es null', () => {
  const match = new Match();
  expect(match.setWinner(null)).toBeFalsy();
  expect(match.winner).toBeNull();
});